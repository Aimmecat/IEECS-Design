...
class NodeToCurrents:
    ...
    def addCurrent(self, current, node, sign):
        if not self.dict.has_key(node):
            self.dict[node] = []
        self.dict[node].append([current,sign])
    ...
    def getKCLEquations(self, gnd):
        ...
        for node in self.dict.keys():
            if not node == gnd:
                (currents, signs) = apply(zip, self.dict[node])
                result.append(le.Equation(signs, currents, 0))
...
class OpAmp(Component):
    ...
    def getEquation(self):
        return le.Equation([self.K,-self.K,-1],
                                    [self.nPlus,self.nMinus,self.nOut],
                                   0)


##test = Circuit([VSrc(15,'n1','n3'),
##                        Resistor(3,'n1','n2'),
##                        Resistor(2,'n2','n3'),
##                        ISrc(10,'n3','n2')])
##test.solve('n3')
