...
class polyR(Signal):
    def __init__(self, s, PolyList):
        self.s = s
        self.PolyList = PolyList.coeffs
        self.sout = Empty()
    def sample(self,n):
        PolyListlen = len(self.PolyList)
        for i in range(PolyListlen):
            self.sout += Rn(self.s,PolyListlen - 1 - i) * self.PolyList[i]
        return self.sout.sample(n)
