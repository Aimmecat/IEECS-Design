...
int main(void)
{
    ...
    // 使用定时器1做编码器解码(带方向编码器模式)
    encoder_init_dir(TIM1_ENCOEDER, TIM1_CH1_ENCOEDER_E9, TIM1_CH2_ENCOEDER_E11);
    // 使用定时器3做编码器解码(带方向编码器模式)
    encoder_init_dir(TIM3_ENCOEDER, TIM3_CH1_ENCOEDER_C6, TIM3_CH2_ENCOEDER_C7);
    // 使用定时器9做编码器解码(带方向编码器模式)
    encoder_init_dir(TIM9_ENCOEDER, TIM9_CH1_ENCOEDER_D9, TIM9_CH2_ENCOEDER_D11);
    // 使用定时器10做编码器解码(带方向编码器模式)
    encoder_init_dir(TIM10_ENCOEDER, TIM10_CH1_ENCOEDER_D1, TIMER10_CH2_ENCOEDER_D3);
